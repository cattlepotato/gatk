...
def writeHdfs(q):
	...
	f = hdfs.open('/user/liucheng/NA12878/NA12878_500w_merge_nameSeq.fastq','wb',block_size=134217728)
	f.close()
	f = hdfs.open('/user/liucheng/NA12878/NA12878_500w_merge_nameSeq.fastq','ab',buff=112197632)
	while 1:
		seqList = q.get()
		print len(seqList)
		if seqList == 'end':
...
def readFile(filename,q_file_seq,q_file_qual):
	f = open(filename,'r')
	seq = ''
	seqList = ''
	id = 1
...
if __name__ == '__main__':
	...
	t_start = time.time()
	q_file1_seq = Queue(5)
	q_file1_qual = Queue(5)
	q_file2_seq = Queue(5)
	q_file2_qual = Queue(5)
	q_merge = Queue(10)
	
	p1 = Process(target=readFile,args=('/tmp/ERR/NA12878_500w_1.fastq',q_file1_seq,q_file1_qual,))	
	p1.start()
	print 'read1 start'

	p2 = Process(target=readFile,args=('/tmp/ERR/NA12878_500w_2.fastq',q_file2_seq,q_file2_qual,))
	p2.start()
	print 'read2 start'
