...
public class AnnotatedVariantProducerUnitTest extends BaseTest {
    ...
    private Object[][] dataForIntegrativeTest() {
        ...
        data.add(new Object[]{forSimpleTanDupContraction_minus, Stream.concat( commonAttributes.stream(),
                Sets.newHashSet(TANDUP_CONTRACTION_STRING, DUP_REPEAT_UNIT_REF_SPAN, DUPLICATION_NUMBERS, HOMOLOGY, HOMOLOGY_LENGTH).stream()).sorted().collect(Collectors.toList()),
                referenceBroadcast});

        // simple tandem dup expansion from 1 unit to 2 units
        data.add(new Object[]{forSimpleTanDupExpansion_plus, Stream.concat( commonAttributes.stream(),
                Sets.newHashSet(TANDUP_EXPANSION_STRING, DUP_REPEAT_UNIT_REF_SPAN, DUP_SEQ_CIGARS, DUPLICATION_NUMBERS).stream()).sorted().collect(Collectors.toList()),
                referenceBroadcast});

        // simple tandem dup expansion from 1 unit to 2 units and novel insertion
        data.add(new Object[]{forSimpleTanDupExpansionWithNovelIns_minus, Stream.concat( commonAttributes.stream(),
                Sets.newHashSet(TANDUP_EXPANSION_STRING, DUP_REPEAT_UNIT_REF_SPAN, DUP_SEQ_CIGARS, DUPLICATION_NUMBERS, INSERTED_SEQUENCE).stream()).sorted().collect(Collectors.toList()),
                referenceBroadcast});

        // tandem dup expansion from 1 unit to 2 units with pseudo-homology
        data.add(new Object[]{forComplexTanDup_1to2_pseudoHom_plus, Stream.concat( commonAttributes.stream(),
                Sets.newHashSet(TANDUP_EXPANSION_STRING, DUP_REPEAT_UNIT_REF_SPAN, DUPLICATION_NUMBERS, DUP_ANNOTATIONS_IMPRECISE, HOMOLOGY, HOMOLOGY_LENGTH).stream()).sorted().collect(Collectors.toList()),
                referenceBroadcast});

        // tandem dup contraction from 2 units to 1 unit with pseudo-homology
        data.add(new Object[]{forComplexTanDup_2to1_pseudoHom_minus, Stream.concat( commonAttributes.stream(),
                Sets.newHashSet(TANDUP_CONTRACTION_STRING, DUP_REPEAT_UNIT_REF_SPAN, DUPLICATION_NUMBERS, DUP_ANNOTATIONS_IMPRECISE, HOMOLOGY, HOMOLOGY_LENGTH).stream()).sorted().collect(Collectors.toList()),
                referenceBroadcast});
